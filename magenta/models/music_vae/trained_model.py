...
class TrainedModel(object):
    ...
    def sample(self, n=None, length=None, temperature=1.0, same_z=False,
        ...
        Raises:
          ValueError: If `length` is not specified and an end token is not being
            used.
        """
        print("SAMPLING FROM IN_MEMORY VAE, setting same_z true for lower variation", self._config, "same_z:", same_z)
        batch_size = self._config.hparams.batch_size
        n = n or batch_size
        z_size = self._config.hparams.z_size
        np.random.seed(0)
        print("SETTING NUMPY SEED FOR REPRODUCIBLE VAE OUTPUT")

        if not length and self._config.data_converter.end_token is None:
            raise ValueError('A length must be specified when the end token is not used.')
        ...
        for _ in range(int(np.ceil(n / batch_size))):
            if self._z_input is not None and not same_z:
                feed_dict[self._z_input] = (np.random.randn(batch_size, z_size).astype(np.float32))
                print("PRINTING FEED DICT STATE PRIOR TO DECODE", feed_dict)
        ...
        if self._c_input is not None:
            return self._config.data_converter.to_items(samples,
                                                        np.tile(np.expand_dims(c_input, 0), [batch_size, 1, 1]))
        else:
            return self._config.data_converter.to_items(samples)
    ...
    def encode_tensors(self, input_tensors, lengths, control_tensors=None):
        ...
        length_array = np.pad(length_array, [(0, batch_pad_amt)] + [(0, 0)] * (length_array.ndim - 1), 'constant')

        max_length = max([len(t) for t in input_tensors])
        print("ENCODE TENSOR LENGTHS:", max_length, len(max_length))
        inputs_array = np.zeros([len(input_tensors), max_length, input_depth])
        for i, t in enumerate(input_tensors):
            inputs_array[i, :len(t)] = t

        control_depth = self._config.data_converter.control_depth
        print("ENCODE CONTROL DEPTH: ", control_depth)
        controls_array = np.zeros([len(input_tensors), max_length, control_depth])
        print("Controls shape: ", controls_array.shape)
        print("Controls: ", controls_array)
        ...
        for i in range(len(inputs_array) // batch_size):
            ...
            outputs.append(self._sess.run([self._z, self._mu, self._sigma], feed_dict))
        assert outputs
    ...
    def decode(self, z, length=None, temperature=1.0, c_input=None):
        ...
        print("LATENT VECTORS DECODED INTO TENSOR FORM")
        for decoded_latent in tensors:
            print("SHAPE: ", decoded_latent.shape, "LATENT_VECTOR: ", decoded_latent)
            nonzero = np.count_nonzero(decoded_latent)
            print("Number of non-zeroes in LATENT VECTOR : ", nonzero)

        if self._c_input is not None:
            return self._config.data_converter.to_items(tensors, np.tile(np.expand_dims(c_input, 0), [self._config.hparams.batch_size, 1, 1]))
        else:
            return self._config.data_converter.to_items(tensors)

    def decode_to_tensors(self, latent_vectors, length=None, temperature=1.0, c_input=None,
                          return_full_results=False):
        """Decodes a collection of latent vectors into output tensors.

        Args:
          latent_vectors: A collection of latent vectors to decode.
          length: The maximum length of a sample in decoder iterations. Required
            if end tokens are not being used.
          temperature: The softmax temperature to use (if applicable).
          c_input: Control sequence (if applicable).
        ...
        print("DECODING PROVIDED TENSORS")
        batch_size = self._config.hparams.batch_size
        sample_count = len(latent_vectors)
        length = length or tf.int32.max

        batch_pad_amt = -sample_count % batch_size
        latent_vectors = np.pad(latent_vectors, [(0, batch_pad_amt), (0, 0)], mode='constant')
        for i, vector in enumerate(latent_vectors):
            print("PRINTING LATENT VECTOR: ", " VECTOR: ", vector, " SHAPE: ", vector.shape)

        outputs = []
        for i in range(len(latent_vectors) // batch_size):
            feed_dict = {
                self._temperature: temperature,
                self._z_input: latent_vectors[i * batch_size:(i + 1) * batch_size],
                self._max_length: length,
            }
            if self._c_input is not None:
                feed_dict[self._c_input] = c_input
            ...
            print("STATE OF OUTPUTS WHEN DECODING LATENT VECTOR: ", i, " OUTPUTS:", outputs)
        return outputs[:sample_count]
